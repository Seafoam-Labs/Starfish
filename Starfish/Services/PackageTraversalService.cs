    ...
    : IPackageTraversalService
{
    ...
    private static string GetBaseName(string dependency)
    {
        var span = dependency.AsSpan();
        var index = span.IndexOfAny(['>', '<', '=', '!']);
        return (index == -1 ? span : span[..index]).Trim().ToString();
    }

    private IEnumerable<AlpmPackageDto> ResolveDependency(string dependency)
    {
        var baseName = GetBaseName(dependency);
        
        if (_packageMap != null && _packageMap.TryGetValue(baseName, out var pkg))
        {
            yield return pkg;
        }

        if (_providesMap == null || !_providesMap.TryGetValue(baseName, out var providers)) yield break;
    ...
    public async Task<Dictionary<string, List<string>>> FetchFullDependencyPackageInformation(string rootPackageName, int depth = 0)
    {
    ...
    public async Task<Dictionary<string, List<string>>> FetchInverseFullDependencyPackageInformation(string rootPackageName, int depth = 0)
    {
        await InitializeAsync();
        var dictionary = new Dictionary<string, List<string>>(StringComparer.OrdinalIgnoreCase);
        var queue = new Queue<(string Name, int CurrentDepth)>();
    ...
    public async Task<Dictionary<string, List<string>>> FetchFullDependencyPackageInformationInstalled(string rootPackageName, int depth = 0)
    {
        await InitializeAsync();
        var dictionary = new Dictionary<string, List<string>>(StringComparer.OrdinalIgnoreCase);
        var queue = new Queue<(string Name, int CurrentDepth)>();
        ...
        while (queue.Count > 0)
        {
            ...
            if (_packageMap == null || !_packageMap.TryGetValue(currentName, out var package) || !_installedNames!.Contains(package.Name))
            {
                continue;
            }
    ...
    public async Task<Dictionary<string, List<string>>> FetchInverseFullDependencyPackageInformationInstalled(string rootPackageName, int depth = 0)
    {
        await InitializeAsync();
        var dictionary = new Dictionary<string, List<string>>(StringComparer.OrdinalIgnoreCase);
        var queue = new Queue<(string Name, int CurrentDepth)>();
    ...
    private async Task InitializeAsync()
    {
        ...
        {
            foreach (var baseProvide in package.Provides.Select(GetBaseName))
            {
